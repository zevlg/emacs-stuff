...
;;{{{ Copyright (C) 2006-2009 by Zajcev Evgeny.

;; Author: Zajcev Evgeny <zelg@yandex.ru>
;; Created: Mon Apr 10 21:13:45 MSD 2006
;; Keywords: math
;; X-CVS: $Id: eukleides.el,v 1.14 2008/12/02 08:08:13 lg Exp $
...
;; eukleides.el is full featured Emacs interface to Eukleides - A
...
;;   - Easy menu
;;   - Contextual smart completion, very smart
;;   - Printing
...
(defvar eukleides-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [(control ?c) (control ?t)] 'eukleides-tex-source)
...
(defun eukleides-arguments-match-p (args kw-args)
  "Return non-nil if ARGS match keyword's arguments KW-ARGS.
...
(defun eukleides-complete-symbols ()
  ...
  (let* ((kw (eukleides-statement-intern (eukleides-statement-at-point)))
         ...
         (types-list nil))
    (if (and kw cmd-args (not (second cmd-args)))
        ;; Single keyword completion
        (list (eukleides-keyword-with-parens kw))

      (dolist (type (mapcar #'car (cddr kw)))
...
(defun eukleides-complete ()
  ...
  (let ((gc-cons-threshold most-positive-fixnum)) ; inhibit gc
    ...
    ;; Move point inside parens
    (when (and eukleides-complete-keyword-with-parens
               (< (point-min) (- (point) 2))
...
curve\\|invalid length\\|syntax error\\)" nil t)
...
(defun eukleides-euk-to-tex (eukfile &optional noaddons)
...
(defun eukleides-tex-source ()
  "Show TeX source for eukleides file."
  (interactive)
  (find-file (eukleides-euk-to-tex (buffer-file-name) t)))
...
(defun eukleides-interactive-increase (arg)
  "Increase interactive value by ARG."
  (interactive "p")
  (unless eukleides-interactive-state
    (if (= (length eukleides-interactive-values) 1)
        (eukleides-interactive-start (caar eukleides-interactive-values))
      (call-interactively #'eukleides-interactive-start)))
...
(defun eukleides-interactive-decrease (arg)
  "Decrease interactive value by ARG."
  (interactive "p")
...
If prefix ARG is supplied then ommit prompting."
  (interactive "P")
