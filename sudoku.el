...
(defcustom sudoku-font-size 24
  "Font size."
  :type 'number
  :group 'sudoku)

(defface sudoku-border-face
  `((t (:size ,sudoku-font-size)))
  "Face for drawing sudoku board borders."
  :group 'sudoku)

(defface sudoku-orig-value-face
  `((t (:bold t :size ,sudoku-font-size)))
  "Face for original values in sudoku field."
  :group 'sudoku)

(defface sudoku-value-face
  `((((type tty) (class color))
     (:bold t :size ,sudoku-font-size :foreground "cyan"))
    (((class color) (background light))
     (:bold t :size ,sudoku-font-size :foreground "darkblue"))
    (((class color) (background dark))
     (:bold t :size ,sudoku-font-size :foreground "blue"))
    (t (:bold t :size ,sudoku-font-size)))
...
(defface sudoku-value-pencil-1-face
  `((((type tty) (class color))
     (:size ,sudoku-font-size :foreground "slategray"))
    (((class color) (background light))
     (:size ,sudoku-font-size :foreground "slategray"))
    (((class color) (background dark))
     (:size ,sudoku-font-size :foreground "slategray"))
    (t (:size ,sudoku-font-size :inverse-video t)))
  "Face used for first pencil values."
  :group 'sudoku)

(defface sudoku-value-pencil-2-face
  `((((type tty) (class color))
     (:size ,sudoku-font-size :foreground "darkgrey"))
    (((class color) (background light))
     (:size ,sudoku-font-size :foreground "darkgrey"))
    (((class color) (background dark))
     (:size ,sudoku-font-size :foreground "darkgrey"))
...
(defface sudoku-autovalue-face
  `((((type tty) (class color))
     (:bold t :size ,sudoku-font-size :foreground "cyan"))
    (((class color) (background light))
     (:bold t :size ,sudoku-font-size :foreground "darkviolet"))
    (((class color) (background dark))
     (:bold t :size ,sudoku-font-size :foreground "mediumpurple"))
    (t (:bold t :size ,sudoku-font-size)))
...
Any style assumes fixed-width font."
  :type '(choice (const :tag "Plain" plain)
                 (const :tag "Pseudo-graphics" pseudo))
  :group 'sudoku)

(defcustom sudoku-blank-cell '((plain . ?\-) (pseudo . ?\x1f))
...
(defun sudoku-custom-instructions ()
  ...
  (list (sudoku-faced-string "Custom puzzle editor" '(underline))
        "Fill in values"
  ...
  (let* ((rcells (remove-if-not #'sudoku-cell-empty-p cells))
         ...
         (combs (mapcan #'(lambda (ar)
                        ...
                        (loop for i from 2 below (length rcells)
                          collect i))))
    (flet ((getpos (cl) (cdr (assoc cl rposss))))
...
(defun sudoku-board-insert-cell (cell)
  (if (or (null cell) (and (numberp cell) (= cell 0)))
    ...
    (insert-face (char-to-string (int-to-char (+ 48 (sudoku-cell-num cell))))
...
If MESSAGE is specified insert it after the board."
  (save-excursion
    ...
    (let* ((style (ecase sudoku-style
                    (plain sudoku-style-plain)
                    (pseudo sudoku-style-pseudo)))
           (hc (make-string 7 (aref style 1))))
      (flet ((inshelp ()
        ...
        (insert-face (format "%c%s%c%s%c%s%c"
                       (aref style 0) hc (aref style 2) hc
                       (aref style 2) hc (aref style 3))
                     'sudoku-border-face)
        (insert "\n")
        (insrow 0) (insrow 1) (insrow 2)
        (insert-face (format "%c%s%c%s%c%s%c"
                       (aref style 4) hc (aref style 6) hc
                       (aref style 6) hc (aref style 8))
                     'sudoku-border-face)
        (inshelp) (insert "\n")
        ...
        (insert-face (format "%c%s%c%s%c%s%c"
                       (aref style 9) hc (aref style 11) hc
                       (aref style 11) hc (aref style 12))
                     'sudoku-border-face)
        (inshelp) (insert "\n")
        (when (sudoku-puzzle-url sudoku-puzzle)
...
Run tests to ensure that the change is a valid one."
  (interactive (list (sudoku-point-genprops
                      (- (char-to-int
                          (event-key (aref (this-command-keys) 0))) 48))))
...
(defun sudoku-faced-string (str face-list)
...
victory, and then asks if you want to play again."
  (sudoku-board-redraw
   nil (append (sudoku-board-instructions)
               (list "" (sudoku-faced-string
                         "YOU WON!" '(sudoku-orig-value-face)))))
  (sudoku-goto-center)
